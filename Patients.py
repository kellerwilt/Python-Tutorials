import datetime #Importe the datetime module we will use to determine what day it is, and do calculations involving dates
...
print "Today is " + ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"][date.weekday()] + ", " + ["January","Fabruary","March","April","May","June","July","August","September","October","November","December"][date.month] + ' ' + str(date.day) + ', ' + str(date.year) #Displays today's date in a prettier format
class PatientsImporter: #Declaring class
    def __init__(self,file): #When we call this function later, 'file' will be Patients.txt
        self.patients = [] #Making a list for us to add each patients data do
        ...
        while unread_lines: #Rinse repeat
            ...
            if len(line) == 1: #determines if the while loop is finished
                unread_lines = False 
                break #ends the loop
            line = map((lambda s: s.strip()), line) #removes trailing spaces
            self.name = line[0]   #stores the data gathered under the following variables
            self.height = line[1]
            self.weight = line[2]
            self.age = line[3]
            self.last_appointment = [int(line[4]),int(line[5]),int(line[6])] #makes a list of the year, month, and day
            ...
            self.until_next_appointment = self.next_appointment - date #calculates timre remaining before next appointment
            if self.until_next_appointment.days <= 0: #determines if an untracked appointment has occured
        ...
        file = open(file,"w") #resets what line of patients.txt the program is on, and opens it in a way that lets it edit the file
...
class Patient:
    ...
    def remind_patients(self): 
        ...
        if self.until_next_appointment.days < 2: #checks to see if the program needs to send a 'tomorrow' reminder
        ...
        if self.until_next_appointment.days < 1: #checks to see if the appointment is today
    ...
    def __str__(self): #Tells the class how I want it to display the data
        ...
        if self.until_next_appointment.days < 31: #If the appointment occurs in less than a month, displays the remaining days until the appointment when displaying the appointments date
            info += "Upcoming appointment: "+str(self.next_appointment) + " in " + str(self.until_next_appointment.days) + " days."
        else:
            info += "Upcoming appointment: "+str(self.next_appointment) #otherwise, prints the date normally
...
for i in patients: #iterates through that list
    print i #prints it out the way we told the program to when defining __str__()
